...
- **코인·주식·투자·자산 관련 모든 요청 → 자산관리사에게 위임 후 보고 수령 → 대표자에게 전달**

### 팀별 에이전트 (1차: 생성만 완료)

| **팀 (Team)** | **에이전트명 (Name)** | **담당 영역 (Scope)** | **업무 (Tasks)** |
|----|-----------|---------|---------| 
...
| **VIP투자팀** | **자산관리사** | **코인·주식 시장 분석·자문, 거시경제 분석, 포트폴리오 관리** | **바이비트(Bybit) API 추후 연동 (CEO 제공 예정)** |
...
| **에이전트 (Agent)** | **실무 처리 (Execution)** | **비서실장 보고 (Reporting)** | **최종 의사결정 (Decisioning)** |
|---------|---------|------------|--------|
| 비서실장 | gemini-3-flash-001 | — | gemini-3.1-pro-001 |
| 기획PM | gemini-3-flash-001 ~ 3.1-pro-001 | gemini-3.1-pro-001 | — |
| 마케터 | gemini-3-flash-001 ~ 3.1-pro-001 | gemini-3.1-pro-001 | — |
| 디자이너 | gemini-3-flash-001 ~ 3.1-pro-001 | gemini-3.1-pro-001 | — |
| 재무담당 | gemini-3-flash-001 ~ 3.1-pro-001 | gemini-3.1-pro-001 | — |
| CS담당 | gemini-3-flash-001 ~ 3.1-pro-001 | gemini-3.1-pro-001 | — |
| **자산관리사** | **gemini-3.1-pro-001 (기본)** | **gemini-3.1-pro-001** | **승인 후 매매 집행** |

> 팀 에이전트 실무 처리는 기본적으로 gemini-3-flash-001을 사용하되, 복잡한 분석이나 높은 품질이 필요한 작업에는 gemini-3.1-pro-001로 자동 업그레이드한다. (단, 예산 허용 범위 내)
> **중요 예외:** VIP 자산관리사는 금융 리스크 관리를 위해 실무에서도 **gemini-3.1-pro-001을 기본 모델로 사용**한다.
...
| 단계 | 작업 | 세부 내용 | 상태 |
|------|------|---------|------|
| 1 | Oracle Cloud A1 세팅 | A1.Flex 인스턴스 생성(4OCPU, 24GB) + 보안그룹 + SSH 키 | ✅ 완료 |
| 2 | Docker 환경 구성 | Docker + Docker Compose 설치 + Nginx + PostgreSQL + OpenClaw 구성 | ✅ 완료 |
| 3 | SSL + 도메인 | Let's Encrypt HTTPS 설정 + 도메인 연결 (또는 OCI 퍼블릭 IP) | ✅ 완료 |
| 4 | 텔레그램 봇 | Bot 생성·Webhook 연결·기본 메시지 구조 | ✅ 완료 |
| 5 | 비서실장 에이전트 | OpenClaw 생성·듀얼모델 라우팅·비서실장-Pro 위임 로직 | ✅ 완료 |
| 6 | 팀 에이전트 ×7 | 7개 에이전트 생성·기본 프롬프트·스킬 배치 완료 (자산관리사 포함) | ✅ 완료 |
| 7 | PostgreSQL 연동 | 처리 결과 저장·메모리 시스템·비용 로그 테이블 | ✅ 완료 |
| 8 | 예산 차단기 구현 | BudgetGuard 미들웨어·알림 시스템·일/월 한도 차단 | ✅ 완료 |
| 9 | 보안 강화 | UFW 방화벽·게이트웨이 토큰·비-root 실행·프롬프트 인젝션 방어·rpcbind 비활성화 | ✅ 완료 |
| 10 | 최종 점검·배포 | 보안 체크리스트 + 유휴 회수 방지 + OCI 예산 알림($0) 설정 | 진행 중 |
| 11 | 자산관리사 API 연동 | CoinGecko·Alpha Vantage·FRED·바이비트 API 키 발급 및 .env 등록 | **예정** |
| 12 | 루틴·업무 할당 | 각 팀 구체적 임무·자동화 규칙 설정 | **추후** |
| 13 | 고도화 | 이미지 생성·KT AI 비서 연동 등 | **추후** |
...
- 게이트웨이 토큰 인증 필수 (auth: none은 2026.02 이후 제거됨) ✅ **[완료 — 게이트웨이 토큰 활성화, 디바이스 페어링 완료]**
- DM 접근 정책: allowlist 모드 — 대표자 텔레그램 ID만 허용 ✅ **[완료]**
- 텔레그램 chat_id 화이트리스트 적용 — 대표자 외 모든 메시지 무시 ✅ **[완료]**

### 12.4 Docker 컨테이너 보안

- 비-root 사용자(UID 1000)로 실행 ✅ **[완료]**
- /shared 볼륨 권한 750으로 설정 (UID 1000 소유) ✅ **[완료]**
...
## 17. 자산관리사 — 필요 API 및 연동 계획

### 17.1 Phase 1 (분석·자문 전용 — 자동매매 API 불필요)

자산관리사는 현재 자문만 제공합니다. 아래 **무료 API**를 권장합니다:

| API | 용도 | 무료 한도 | 우선순위 |
|-----|------|----------|---------|
| **CoinGecko API** | 코인 시세·시총·거래량·차트 | 월 10,000회 무료 | ★★★ 필수 |
| **Alpha Vantage** | 주식 (국내·해외)·환율·경제지표 (GDP, CPI 등) | 일 25회 무료 | ★★★ 필수 |
| **FRED API** (미연준) | 미국 거시경제: 금리·물가·실업률·M2 통화량 | 무제한 무료 | ★★★ 필수 |
| **CryptoCompare** | 코인 시장 데이터·뉴스·포트폴리오 | 월 100,000회 무료 | ★★ 권장 |
| **Finnhub** | 실시간 주식 시세·실적 발표 일정·뉴스 | 분당 60회 무료 | ★★ 권장 |

### 17.2 Phase 2 (CEO가 API 키 제공 시 — 자동매매)

| API | 용도 | 비고 |
|-----|------|------|
| **Bybit API** (실계정 + 테스트넷) | 코인 현물/선물 매매·잔고·주문 | CEO가 `BYBIT_API_KEY` + `BYBIT_SECRET` 제공 예정 |
| **한국투자증권 (KIS) API** | 국내 주식 자동매매 | CEO 준비 시 제공 예정 |

### 17.3 .env에 추가할 키 (Phase 1 — 지금 발급 가능)

```bash
# 자산관리사 — 시장 데이터 API
COINGECKO_API_KEY=          # 선택 (키 없이도 동작, 키 있으면 한도 증가)
                            # 발급: https://www.coingecko.com/en/api/pricing
ALPHA_VANTAGE_API_KEY=      # ★ 필수 — 무료 발급: https://www.alphavantage.co/support/#api-key
FRED_API_KEY=               # ★ 필수 — 무료 발급: https://fred.stlouisfed.org/docs/api/api_key.html
CRYPTOCOMPARE_API_KEY=      # 선택 — https://min-api.cryptocompare.com/

# 자산관리사 — 거래소 API (Phase 2 — CEO가 추후 제공)
# BYBIT_API_KEY=
# BYBIT_SECRET=
```

### 17.4 자산관리사 커뮤니케이션 흐름

```
대표자(CEO)
  ↓ [텔레그램]
비서실장
  ↓ [sessions_send agentId: "asset-manager"]
자산관리사 (분석·자문·추후 자동매매)
  ↓ [업무 완료 후 보고]
비서실장
  ↓ [결과 정리·전달]
