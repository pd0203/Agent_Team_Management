...
> ⚠️ **절대 금지**: `git` 명령을 직접 실행하지 마십시오.
> 당신의 워크스페이스(`/home/node/.openclaw/workspace/`)에는 git 저장소가 없습니다.
> git 저장소는 별도의 `agent-git-push` 컨테이너가 전담합니다.

**1단계: 파일 수정** — OpenClaw 기본 파일 도구로 대상 파일을 수정

**2단계: GitHub Push** — bash 도구로 아래 curl 명령을 실행:
```bash
...
> 💡 git add / git commit / git push 명령 불필요 — curl 한 줄로 전부 처리됩니다.
