...
> 🚨 **핵심 규칙** — 반드시 숙지하십시오:
> - `git` 명령은 **당신의 환경에서 절대 작동하지 않습니다** (git 저장소 없음)
> - `.git` 폴더 탐색, `git status`, `git init` 등 **모든 git 명령 시도 금지**
> - GitHub push는 **오직 아래 curl 한 줄**로만 처리됩니다. 다른 방법 없음.
> - git 환경 "확인"이나 "검증" 절차 불필요 — curl 응답이 곧 성공/실패 확인입니다.
